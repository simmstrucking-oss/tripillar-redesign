...
async function sendEmail(name: string, email: string, label: string, message: string) {
  ...
  await transporter.sendMail({
    ...
    text: [`Name: ${name}`, `Email: ${email}`, `Inquiry Type: ${label}`, ``, `Message:`, message, ``, `---`, `Submitted via tripillarstudio.com/contact`].join("\n"),
    html: `
...
export async function POST(req: NextRequest) {
  ...
  // 1. Save to Supabase (ensures nothing lost even if email fails)
  ...
  try {
    await sendEmail(name, email, label, message);
    // Mark emailed
    if (submissionId) {
    ...
    // Save error to DB but DO NOT fail the user response — submission is saved
    if (submissionId) {
      supabase.from("contact_submissions").update({ email_error: emailError }).eq("id", submissionId).then(() => {});
    }
    // If SMTP fails AND Supabase also failed, return error
    if (!submissionId) {
    ...
    // If saved to Supabase, still return success — Wayne will see it in DB
    // But also try to notify via Kit tag (so Wayne gets Kit email)
  }
